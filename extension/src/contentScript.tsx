...
async function fetchTranscriptFromDOM(videoId: string): Promise<string | null> {
  try {
    ...
    if (transcriptButton) {
      ...
      if (transcriptSegments.length > 0) {
        ...
        if (fullTranscript.length > 0) {
          ...
          console.warn('Found transcript segments, but could not extract text or timestamps from any of them.');
          return null;
        }
      } else {
        console.warn('Transcript panel might be open, but no transcript segments (ytd-transcript-segment-renderer) found.');
        return null;
      }
    } else {
      console.warn('"Show transcript" button not found on the page.');
      return null;
    }
  } catch (error) {
    console.error('Error in fetchTranscriptFromDOM:', error);
    return null;
  }
};
  ...
  try {
    ...
    try {
      ...
      if (fullTranscript) {
        ...
        logDebug('Retrieved transcript segment for snapshot description');
      }
    } catch (error) {
  ...
  if (appendedContainer) {
    ...
    try {
      ...
      (window as CustomWindow).handleSeekTo = (time: number) => {
        // videoElement is already available in this scope and checked for null
        if (videoElement) {
          videoElement.currentTime = time;
        }
      };
            ...
            if (savedOutline && savedOutline.items && savedOutline.items.length > 0) {
              ...
              return new Promise<void>((resolveStorageGet) => {
                chrome.storage.local.get({ snapshots: [] }, ({ snapshots }: { snapshots: Snapshot[] }) => {
                  ...
                  if (existingSnapshots.length === 0 && snapshotInfos.length > 0) {
                    logDebug(`Creating ${snapshotInfos.length} snapshots from outline items`);
                    setTimeout(() => {
                      handleCaptureAllSnapshots(snapshotInfos);
                      resolveStorageGet(); // Resolve after snapshot capture attempt
                    }, 1000);
                  } else {
                    logDebug('Using existing snapshots, not generating new ones');
                    currentSnapshots = existingSnapshots;
                    if (typeof renderPanel === 'function') {
                      renderPanel(currentIsPlaying, currentSnapshots);
                    }
                    resolveStorageGet(); // Resolve if not capturing new snapshots
                  }
                });
              });
            } else {
