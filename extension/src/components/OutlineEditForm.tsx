...
export const textToOutlineItems = (text: string): { items: OutlineItem[], errors: string[] } => {
  ...
  while (lineIndex < lines.length) {
    ...
    if (timestampMatch) {
      ...
      if (timestamp === null) {
        errors.push(`無効なタイムスタンプ形式: ${timestampStr}`);
        currentItem = null;
      } else {
        currentItem = {
          timestamp,
...
const OutlineEditForm: React.FC<OutlineEditFormProps> = ({ items, onSave, onCancel }) => {
  ...
  const handleSave = useCallback(async () => {
    ...
    if (parsedItems.length === 0) {
      setErrors([t('no_outline_items', 'アウトラインアイテムが見つかりません。')]);
      return;
    }
    ...
    try {
      ...
      setErrors([`${t('save_error', '保存中にエラーが発生しました')}: ${error}`]);
    } finally {
      setIsSaving(false);
    }
  }, [editText, onSave]);
  ...
  return (
    <div className="outline-edit-form" style={{ padding: '16px' }}>
      <h3 style={{ marginBottom: '8px', color: '#333' }}>
        {t('edit_outline', 'アウトラインを編集')}
      </h3>
      
      <div style={{ marginBottom: '8px' }}>
        <p style={{ fontSize: '12px', color: '#666', marginBottom: '4px' }}>
          {t('outline_format_instruction', '形式: [MM:SS]形式のタイムスタンプの後にタイトル、続けて説明文（省略可）')}
        </p>
        <code style={{ display: 'block', fontSize: '12px', backgroundColor: '#f5f5f5', padding: '4px', borderRadius: '4px' }}>
          [MM:SS] {t('title_label', 'タイトル')}<br />
          {t('description_optional', '説明文（省略可）')}
        </code>
      </div>
      ...
      {errors.length > 0 && (
        <div style={{ marginTop: '8px', color: '#d32f2f', fontSize: '14px' }}>
          <p style={{ fontWeight: 'bold', marginBottom: '4px' }}>{t('error_prefix', 'エラー:')} </p>
      ...
      <div style={{ marginTop: '16px', display: 'flex', justifyContent: 'flex-end', gap: '8px' }}>
        ...
        <button
          ...
          disabled={isSaving}
        >
          {isSaving ? t('saving', '保存中...') : t('overlay_save', '保存')}
        </button>
      </div>
    </div>
  );
